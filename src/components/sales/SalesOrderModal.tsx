...
export default function SalesOrderModal({ businessId, order, onClose }: Props) {
  const create = useCreateSalesOrder(businessId);
  const update = useUpdateSalesOrder(businessId, order?.id || '');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white">
          <h2 className="text-xl font-bold text-gray-900">{isEdit ? 'Modifier la commande' : 'Nouvelle Commande'}</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-5" noValidate>
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              {isSubmitting ? (isEdit ? 'Modification...' : 'Création...') : (isEdit ? 'Modifier' : 'Créer la commande')}
            </button>
          </div>
        </form>
      </div>
