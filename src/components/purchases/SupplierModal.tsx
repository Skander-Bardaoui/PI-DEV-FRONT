    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4" noValidate>
          ...
          <Field label="Nom du fournisseur" error={errors.name?.message} required>
            <input {...register('name')} className={inputCls(errors.name?.message)}
              placeholder="Société Exemple SARL" />
          </Field>

          <div className="grid grid-cols-2 gap-4">
            <Field label="Matricule fiscal" error={errors.matricule_fiscal?.message}>
          ...
          <div>
            ...
            <input {...register('address.street')} className={`${inputCls()} mb-2`} placeholder="Rue" />
            <div className="grid grid-cols-3 gap-2">
              <input {...register('address.city')}        className={inputCls()} placeholder="Ville" />
              <input {...register('address.postal_code')} className={inputCls()} placeholder="Code postal" />
              <input {...register('address.country')}     className={inputCls()} placeholder="Pays" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Banque" error={errors.bank_name?.message}>
